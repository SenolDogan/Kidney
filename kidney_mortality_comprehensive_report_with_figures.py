...
def create_comprehensive_report_with_figures():
    ...
    date_paragraph.add_run(f'Report Generated: {datetime.now().strftime("%B %d, %Y")}').italic = True
    ...
    # Add univariate analysis figures
    if os.path.exists('ttest_top20.png'):
        doc.add_paragraph('Figure 1: Top 20 Features by t-test (Numerical Variables)')
        doc.add_picture('ttest_top20.png', width=Inches(6))
        doc.add_paragraph('This figure shows the most significant numerical variables based on t-test analysis comparing survivors vs non-survivors.')
    
    if os.path.exists('chi_square_top20.png'):
        doc.add_paragraph('Figure 2: Top 20 Features by Chi-square Test (Categorical Variables)')
        doc.add_picture('chi_square_top20.png', width=Inches(6))
        doc.add_paragraph('This figure shows the most significant categorical variables based on chi-square test analysis.')
    ...
    # Add machine learning figures
    if os.path.exists('roc_rf_xgb.png'):
        doc.add_paragraph('Figure 3: ROC Curves Comparison - Random Forest vs XGBoost')
        doc.add_picture('roc_rf_xgb.png', width=Inches(6))
        doc.add_paragraph('ROC curves comparing the performance of Random Forest and XGBoost models.')
    
    if os.path.exists('rf_feature_importance.png'):
        doc.add_paragraph('Figure 4: Random Forest Feature Importance')
        doc.add_picture('rf_feature_importance.png', width=Inches(6))
        doc.add_paragraph('Feature importance ranking from Random Forest model showing the most predictive variables.')
    
    if os.path.exists('xgb_feature_importance.png'):
        doc.add_paragraph('Figure 5: XGBoost Feature Importance')
        doc.add_picture('xgb_feature_importance.png', width=Inches(6))
        doc.add_paragraph('Feature importance ranking from XGBoost model showing the most predictive variables.')
    ...
    # Add survival analysis figures
    if os.path.exists('km_A_Body_Shape_Index_ABSI_2groups.png'):
        doc.add_paragraph('Figure 6: Kaplan-Meier Survival Curves - Body Shape Index (ABSI)')
        doc.add_picture('km_A_Body_Shape_Index_ABSI_2groups.png', width=Inches(6))
        doc.add_paragraph('Kaplan-Meier survival curves comparing patients who died within 1 year vs after 1 year, stratified by Body Shape Index.')
    
    if os.path.exists('km_eGFR_CKD_EPI_Creatinine_at_Baseline_2groups.png'):
        doc.add_paragraph('Figure 7: Kaplan-Meier Survival Curves - eGFR')
        doc.add_picture('km_eGFR_CKD_EPI_Creatinine_at_Baseline_2groups.png', width=Inches(6))
        doc.add_paragraph('Kaplan-Meier survival curves comparing patients who died within 1 year vs after 1 year, stratified by eGFR levels.')
    ...
    if os.path.exists('clustering_results/kmeans_pca_with_centroids.png'):
        doc.add_paragraph('Figure 8: K-means Clustering Results with Centroids')
        doc.add_picture('clustering_results/kmeans_pca_with_centroids.png', width=Inches(6))
        doc.add_paragraph('PCA visualization of K-means clustering results showing 3 distinct patient groups with cluster centroids marked in red.')
    
    if os.path.exists('clustering_results/hierarchical_dendrogram.png'):
        doc.add_paragraph('Figure 9: Hierarchical Clustering Dendrogram')
        doc.add_picture('clustering_results/hierarchical_dendrogram.png', width=Inches(6))
        doc.add_paragraph('Dendrogram showing the hierarchical clustering structure of patients based on clinical parameters.')
    ...
    # Add risk stratification figures
    if os.path.exists('risk_stratification_results/top5_deathrate_61+_Male.png'):
        doc.add_paragraph('Figure 10: Top 5 Risk Factors - Males 61+ Years')
        doc.add_picture('risk_stratification_results/top5_deathrate_61+_Male.png', width=Inches(6))
        doc.add_paragraph('Top 5 mortality risk factors specifically for male patients aged 61 years and older.')
    ...
    # Save the document
    doc.save('Kidney_Mortality_Comprehensive_Report_with_Figures.docx')
    print("Comprehensive report with figures saved as 'Kidney_Mortality_Comprehensive_Report_with_Figures.docx'")

if __name__ == "__main__":
    create_comprehensive_report_with_figures() 